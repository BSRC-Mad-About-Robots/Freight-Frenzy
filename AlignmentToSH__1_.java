...
public class AlignmentToSH__1_ extends LinearOpMode {
    ...
    public void runOpMode()
    {
      ...
      int cameraMonitorViewId = hardwareMap.appContext.getResources().getIdentifier("cameraMonitorViewId", "id", hardwareMap.appContext.getPackageName());
        ...
        double currentarea=detector.getArea();
        inchestogo=(AreabyInch*(TargetPositionArea1-currentarea));
        ...
        setTargetPosition(-((int)(inchestogo*79))-420);
        while(FL.isBusy() && FR.isBusy() && BL.isBusy() && BR.isBusy()){       
            
            
            if(TargetPositionY>final_valY)
            {   Inchestomove=(final_valY+TargetPositionY/Pixelspeed)-0.3;
                // Inchestomove=(-(Inchestomove));
            }
            else{
                Inchestomove=(final_valY/Pixelspeed)+0.3;
            }
            telemetry.addData("Position",Inchestomove);
            telemetry.update();
            // sleep(8000);
         
            
            FR.setPower((0.7-(Inchestomove+0.3))/3);
            FL.setPower(0.7/3);
            BR.setPower(0.7/3);
            BL.setPower((0.7-(Inchestomove+0.3))/3);
