...
@Autonomous
public class Park_In_StorageUnit extends LinearOpMode {
    // Handle hardware stuff...

    int width = 960;
    int height = 720;//changed this
    // store as variable here so we can access the location
    TestOpencv_Copy3 detector = new TestOpencv_Copy3(telemetry);
    ...
    //                         Parl.setPower(0.0);
        ...
        inchestogo=Math.abs((706.0-currentarea)/60.5);
        telemetry.addData("AreabyInch",inchestogo);
        telemetry.addData("AreabyInch",currentarea);
        double adjacent=Math.abs((850.0-final_valY))/80;
        ...
        double newPow=Math.abs((angle))/90;
    ...
    //   while(FL.isBusy()){
          ...
          {
              FL.setPower(1.0/2);
      FR.setPower((newPow)/2);
      BL.setPower((newPow)/2);
      BR.setPower(1.0/2);
      
          }
          else{
              FL.setPower(newPow/2);
      FR.setPower(1.0/2);
      BL.setPower(1.0/2);
      BR.setPower(newPow/2);
...
while(FL.isBusy()) {}
    ...
    //   }
        ...
        inchestogo = Math.abs(712-currentarea)/12.8;
        telemetry.addData("movement", inchestogo);
        telemetry.update();
      FL.setDirection(DcMotorSimple.Direction.REVERSE);
      FR.setDirection(DcMotorSimple.Direction.FORWARD);
      BL.setDirection(DcMotorSimple.Direction.REVERSE);
